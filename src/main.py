...
TEMPLATE_DEFAULT = util.load_html('default')
TEMPLATE_REDIRECT = util.load_html('redirect')
...
def parse_page(context: Context, entry_id: str, buffer: List[str], data: Any):
    ...
    if 'anchor' in data:
        buffer.append('<a class="anchor" id="anchor-%s"></a>' % data['anchor'])
...
def build_book_html(context: Context):
    ...
    # Main Page
    util.write_html(context.output_dir, html=TEMPLATE_DEFAULT.format(
    ...
    for category_id, cat in context.sorted_categories:
        util.write_html(context.output_dir, category_id, html=TEMPLATE_DEFAULT.format(
            title=context.translate(I18n.TITLE),
            text_index=context.translate(I18n.INDEX),
            text_contents=context.translate(I18n.CONTENTS),
            text_version=context.translate(I18n.VERSION),
            ...
            contents='\n'.join([
                '<li><a class="text-muted" href="../%s/">%s</a></li>' % (cat_id, cat.name) + (
                    ...
                    '<ul class="list-unstyled push-right">%s</ul>' % '\n'.join([
                        '<li><a class="text-muted" href="./%s">%s</a></li>' % (ent.rel_id, ent.name)
                        for _, ent in cat.sorted_entries 
                    ])
                )
                for cat_id, cat in context.sorted_categories
            ]),
        ...
        for entry_id, entry in cat.sorted_entries:
            util.write_html(context.output_dir, entry_id, html=TEMPLATE_DEFAULT.format(
                title=context.translate(I18n.TITLE),
                text_index=context.translate(I18n.INDEX),
                text_contents=context.translate(I18n.CONTENTS),
                text_version=context.translate(I18n.VERSION),
                ...
                current_lang=context.translate(I18n.LANGUAGE_NAME % context.lang),
                langs='\n'.join([
                    '<a href="../../../%s/%s/" class="dropdown-item">%s</a>' % (l, entry_id, context.translate(I18n.LANGUAGE_NAME % l)) for l in versions.LANGUAGES
                ]),
                index='../../',
                style='../../../style.css',
                tfc_version=versions.VERSION,
